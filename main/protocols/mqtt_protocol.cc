...
bool MqttProtocol::SendAudio(const AudioStreamPacket& packet) {
    ...
    // Nonce carries runtime packet metadata used by server for ordering/AEC.
    ...
    *(uint32_t*)&nonce[12] = htonl(++local_sequence_);
    ...
    // Audio payload is encrypted with AES-CTR; header/nonce stays in clear text.
    size_t nc_off = 0;
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    // MQTT hello requests UDP audio tunnel parameters (server/port/key/nonce).
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        // Basic anti-replay / ordering guard.
...
void MqttProtocol::ParseServerHello(const cJSON* root) {
    // MQTT control plane + UDP media plane negotiation happens here.
    // Missing UDP block means channel open is incomplete.
