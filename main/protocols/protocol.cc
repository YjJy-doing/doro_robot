...
void Protocol::SendStartListening(ListeningMode mode) {
    // Unified listen control frame used by both WebSocket and MQTT control plane.
...
void Protocol::SendIotDescriptors(const std::string& descriptors) {
    ...
    // Send descriptor one-by-one to keep each JSON frame small and avoid
    // exceeding broker/frame limits on constrained transports.
...
void Protocol::SendMcpMessage(const std::string& payload) {
    // MCP is tunneled inside the project's outer message envelope.
