...
#define LIGHT_BACKGROUND_COLOR       lv_color_white()           // white background--Status bar    
#define LIGHT_TEXT_COLOR             lv_color_hex(0xFF69B4)     // pink text
#define LIGHT_CHAT_BACKGROUND_COLOR  lv_color_white()           // White background-- subface   
#define LIGHT_USER_BUBBLE_COLOR      lv_color_hex(0x95EC69)     // WeChat green
#define LIGHT_ASSISTANT_BUBBLE_COLOR lv_color_white()           // White
#define LIGHT_SYSTEM_BUBBLE_COLOR    lv_color_hex(0xE0E0E0)     // Light gray
#define LIGHT_SYSTEM_TEXT_COLOR      lv_color_hex(0x666666)     // Dark gray text
#define LIGHT_BORDER_COLOR           lv_color_hex(0xFF69B4)     // pink border
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_text_color(status_bar_, current_theme_.text, 0);
    lv_obj_align(status_bar_, LV_ALIGN_TOP_MID, 0, 0);// 将状态栏固定在屏幕顶部
    lv_obj_move_foreground(status_bar_);// 确保状态栏在顶层显示
    ...
    /* 聊天消息标签 - 直接创建在屏幕上，确保背景透明有效 */
    chat_message_label_ = lv_label_create(screen); 
    lv_label_set_text(chat_message_label_, "");
    lv_obj_set_width(chat_message_label_, LV_HOR_RES * 0.9); // 限制宽度为屏幕宽度的 90%
    lv_obj_set_height(chat_message_label_, fonts_.text_font->line_height * 3); // 固定高度为三行
    lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_WRAP); // 设置为自动换行模式
    lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER, 0); // 设置文本居中对齐
    lv_obj_set_style_text_color(chat_message_label_, current_theme_.text, 0);
    lv_obj_set_style_bg_opa(chat_message_label_, LV_OPA_0, 0); // 设置背景透明
    lv_obj_align(chat_message_label_, LV_ALIGN_BOTTOM_MID, 0, 0);// 将聊天消息标签固定在屏幕底部
    lv_obj_move_foreground(chat_message_label_);// 确保聊天消息在顶层显示（在emotion_label_上层）
