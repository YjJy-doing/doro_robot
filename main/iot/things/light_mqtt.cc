...
class LightMqtt : public Thing {
 private:
  esp_mqtt_client_handle_t client_ = nullptr;
  bool mqtt_connected_ = false;
  bool light_on_ = false;
  const char* topic_ = "syj_connect_esp01s_light_topic";
  ...
  static void mqtt_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id, void* event_data) {
    LightMqtt* self = static_cast<LightMqtt*>(handler_args);
    ...
    switch (event_id) {
      ...
      case MQTT_EVENT_DATA: {
        ...
        if (payload == "180") {
          self->light_on_ = true;
          ESP_LOGI(TAG, "[网络状态] 灯已打开");
        } else if (payload == "0") {
          self->light_on_ = false;
          ESP_LOGI(TAG, "[网络状态] 灯已关闭");
        }
        break;
      }
  ...
  static void mqtt_task(void* pvParameters) {
    LightMqtt* self = static_cast<LightMqtt*>(pvParameters);
    self->RunMqttTask();
  }
  ...
  void mqtt_app_start(void) {
    ...
    mqtt_cfg.credentials.client_id = "esp32_light";
 ...
 public:
  LightMqtt()
      : Thing("LightMqtt", "通过MQTT远程获取和控制灯的状态") {
    wifi_event_group = xEventGroupCreate();
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, NULL));
    xTaskCreate(mqtt_task, "mqtt_task", 4096, this, 5, NULL);
    properties_.AddBooleanProperty("state", "灯状态：true-开，false-关", [this]() -> bool {
      return light_on_;
    });
    methods_.AddMethod("SetLightState", "设置灯状态", ParameterList({
      Parameter("state", "灯状态：0-关，1-开", kValueTypeNumber, true)
    }), [this](const ParameterList& parameters) {
      int state = parameters["state"].number();
      ESP_LOGI(TAG, "收到设置命令: %d", state);
      light_on_ = (state == 1);
      SendMqttMessage(state == 1 ? "0" : "180");
    });
    methods_.AddMethod("turn_on", "开灯", ParameterList(), [this](const ParameterList& parameters) {
      ESP_LOGI(TAG, "收到开灯命令");
      light_on_ = true;
      SendMqttMessage("0");
    });
    methods_.AddMethod("turn_off", "关灯", ParameterList(), [this](const ParameterList& parameters) {
      ESP_LOGI(TAG, "收到关灯命令");
      light_on_ = false;
      SendMqttMessage("180");
    });
