...
static bool ContainsUtf8Keyword(const char* text, const char* keyword) {
    if (text == nullptr || keyword == nullptr) {
        return false;
    }
    return std::strstr(text, keyword) != nullptr;
}
...
void Application::Start() {
    ...
    // Initialize transport protocol after OTA config is pulled.
    // Priority: OTA-specified MQTT > OTA-specified WebSocket > MQTT fallback.
    ...
    // All protocol JSON callbacks are translated into scheduled main-loop tasks
    // when they touch shared state/UI. This avoids cross-thread state races.
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                Schedule([this]() {
                    background_task_->WaitForCompletion();
                    bool should_show_talent = pending_talent_after_song_;
                    pending_talent_after_song_ = false;
                ...
                if (cJSON_IsString(text)) {
                    ...
                    if (ContainsUtf8Keyword(text->valuestring, "唱") || ContainsUtf8Keyword(text->valuestring, "歌曲")) {
                        pending_talent_after_song_ = true;
                    }
            ...
            if (cJSON_IsString(text)) {
                ESP_LOGI(TAG, ">> %s", text->valuestring);
                if (ContainsUtf8Keyword(text->valuestring, "唱歌") || ContainsUtf8Keyword(text->valuestring, "歌曲")) {
                    pending_talent_after_song_ = true;
                }
                Schedule([this, display, message = std::string(text->valuestring)]() {
    ...
    // AudioProcessor output path:
    // PCM -> background opus encode -> enqueue SEND_AUDIO_EVENT -> protocol send.
    ...
    // Wake-word callback is hot path: keep logic minimal and schedule heavy work.
...
void Application::OnClockTimer() {
    ...
    if (device_state_ == kDeviceStateIdle && idle_action_countdown_s_ > 0) {
        --idle_action_countdown_s_;
        if (idle_action_countdown_s_ == 0) {
            ...
            idle_action_countdown_s_ = (rand() % 121) + 60;
        }
    }
}
...
void Application::MainEventLoop() {
    ...
    while (true) {
        ...
        if (bits & SEND_AUDIO_EVENT) {
            // Move queue out under lock, then send without lock to keep producer fast.
            std::unique_lock<std::mutex> lock(mutex_);
        ...
        if (bits & SCHEDULE_EVENT) {
            // Execute deferred state/UI mutations on a single serialized thread.
            std::unique_lock<std::mutex> lock(mutex_);
...
void Application::OnAudioOutput() {
    ...
    // Pop exactly one packet each tick to balance decode latency and CPU usage.
    ...
    // Decode is delegated to BackgroundTask to keep AudioLoop non-blocking.
...
void Application::OnAudioInput() {
    // Priority order:
    // 1) audio testing capture
    // 2) wake-word feed
    // 3) conversational audio processor feed
    // This prevents wake-word/test streams from being starved by normal VAD flow.
    if (device_state_ == kDeviceStateAudioTesting) {
...
void Application::SetDeviceState(DeviceState state) {
    ...
    // State transition barrier: wait pending background jobs before mutating
    // decoder/processor/wake-word pipelines to avoid mixed old/new state behavior.
    background_task_->WaitForCompletion();
    ...
    switch (state) {
        ...
        case kDeviceStateListening:
            ...
            // Enter "mic capture -> VAD/AFE -> encode -> uplink" mode.
            if (!audio_processor_->IsRunning()) {
        ...
        case kDeviceStateSpeaking:
            ...
            // Reset decoder queue before new TTS stream to avoid stale tail audio.
            ResetDecoder();
            break;
        default:
            idle_action_countdown_s_ = 0;
